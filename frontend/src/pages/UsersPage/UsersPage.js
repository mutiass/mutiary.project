...
export default function UsersPage() {
  ...
  // Gunakan useCallback untuk mencegah perubahan fungsi yang tidak perlu
  const loadUsers = useCallback(async () => {
    const users = await getAll(searchTerm);
    setUsers(users);
  }, [searchTerm]); // Hanya tergantung pada searchTerm

  useEffect(() => {
    loadUsers();
  }, [loadUsers]); // Tidak ada warning lagi karena loadUsers stabil
